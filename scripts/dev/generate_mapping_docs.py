#!/usr/bin/env python3
"""Generate qualifier mapping documentation pages from mapping-data.json."""
...
OUTPUT_DIR = PROJECT_ROOT / "docs" / "site" / "docs" / "mappings"


def load_mapping_data() -> dict[str, object]:
    with open(MAPPING_DATA_PATH, encoding="utf-8") as f:
        return json.load(f)
...
def get_related_commands(
    ...
    for command_key, command_entry in commands.items():
        if isinstance(command_entry, dict):
            if command_entry.get("qualifier") == qualifier:
...
def render_value_map_section(
    ...
    for attr_name in sorted(value_map.keys()):
        ...
        if not isinstance(attr_values, dict):
            continue
        rows = []
        for left, right in sorted(attr_values.items()):
            if isinstance(right, str):
                rows.append(f"| `{left}` | `{right}` |")
        if rows:
            if not lines:
...
def render_key_value_map_section(
    ...
    for attr_name in sorted(key_value_map.keys()):
        ...
        if not isinstance(attr_values, dict):
            continue
        rows = []
        for friendly_value, mapping in sorted(attr_values.items()):
            if isinstance(mapping, dict):
                mqsc_key = mapping.get("key", "")
                mqsc_value = mapping.get("value", "")
                rows.append(f"| `{friendly_value}` | `{mqsc_key}` | `{mqsc_value}` |")
        if rows:
            if not lines:
            ...
            lines.append(f"### {attr_name}")
            lines.append("")
            lines.append("| Friendly value | MQSC key | MQSC value |")
            lines.append("| --- | --- | --- |")
            lines.extend(rows)
            lines.append("")
    return lines
...
def qualifier_has_mappings(qualifier_entry: dict[str, object]) -> bool:
    mapping_keys = (
        "request_key_map",
        "response_key_map",
        "request_value_map",
    ...
    return any(
        isinstance(qualifier_entry.get(k), dict) and qualifier_entry.get(k)
...
def generate_qualifier_page(
    qualifier: str,
    qualifier_entry: dict[str, object],
    ...
    if isinstance(request_key_map, dict) and request_key_map:
        table = render_key_map_table(request_key_map, "Friendly name", "MQSC parameter")
        if table:
    ...
    if isinstance(response_key_map, dict) and response_key_map:
        table = render_key_map_table(response_key_map, "MQSC parameter", "Friendly name")
        if table:
    ...
    if isinstance(request_value_map, dict) and request_value_map:
    ...
    if isinstance(response_value_map, dict) and response_value_map:
        section = render_value_map_section(
            response_value_map,
            "Response value map",
            "MQSC value",
    ...
    if isinstance(request_key_value_map, dict) and request_key_value_map:
...
def generate_index_page(mapped: list[str]) -> str:
    lines = [
    ...
    for q in sorted(mapped):
        lines.append(f"- [{q}]({q}.md)")
    lines.append("")
...
def main() -> None:
    ...
    if not isinstance(qualifiers, dict):
        print("No qualifiers found in mapping data")
        return
    ...
    for qualifier_name in sorted(qualifiers.keys()):
        ...
        if not isinstance(qualifier_entry, dict):
            continue
        if not qualifier_has_mappings(qualifier_entry):
            continue
        related = get_related_commands(commands, qualifier_name)
        page_content = generate_qualifier_page(qualifier_name, qualifier_entry, related)
